...
const dummyNOCRequests: NOCRequest[] = [
  ...
  },
  {
    ...
    rejectionReason: "Incomplete offer letter.",
  },
  {
    id: "noc4",
...
export function NOCManagement() {
  ...
  const handleApprove = (id: string) => {
    setNocRequests((prev) =>
      prev.map((noc) => (noc.id === id ? { ...noc, status: "approved_teacher", rejectionReason: undefined } : noc)),
    )
    toast.success("NOC Approved", {
      description: `NOC Request ID: ${id} has been approved and forwarded to TPO.`,
    })
  ...
  const handleReject = (id: string) => {
    ...
    setNocRequests((prev) =>
      prev.map((noc) => (noc.id === id ? { ...noc, status: "rejected_teacher", rejectionReason } : noc)),
    )
  ...
  const getFilteredNOCs = (filterStatus: string) => {
    ...
    return nocRequests.filter((noc) => {
      if (filterStatus === "pending") {
        return noc.status === "pending_teacher"
      }
      if (filterStatus === "approved") {
        return noc.status === "approved_teacher" || noc.status === "approved" || noc.status === "pending_tpo"
      }
      if (filterStatus === "rejected") {
        return noc.status === "rejected_teacher" || noc.status === "rejected"
      }
      return false
    })
  }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Review and manage No Objection Certificate requests from your students.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-6 flex flex-wrap gap-2">
          <Button variant={activeTab === "pending" ? "default" : "outline"} onClick={() => setActiveTab("pending")}>
            Pending Your Review ({getFilteredNOCs("pending").length})
          </Button>
