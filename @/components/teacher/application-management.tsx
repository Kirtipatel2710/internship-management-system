...
import { CheckCircle2, XCircle, FileText, User, Briefcase, Mail, Calendar } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
const dummyApplications: Application[] = [
  ...
  },
  {
    ...
    rejectionReason: "Resume missing key project experience.",
  },
  {
    id: "app4",
...
export function ApplicationManagement() {
  ...
  const handleApprove = (id: string) => {
    setApplications((prev) =>
      prev.map((app) => (app.id === id ? { ...app, status: "approved_teacher", rejectionReason: undefined } : app)),
    )
    toast.success("Application Approved", {
      description: `Application ID: ${id} has been approved and forwarded to TPO.`,
    })
  ...
  const handleReject = (id: string) => {
    ...
    setApplications((prev) =>
      prev.map((app) => (app.id === id ? { ...app, status: "rejected_teacher", rejectionReason } : app)),
    )
  ...
  const getFilteredApplications = (filterStatus: string) => {
    ...
    return applications.filter((app) => {
      if (filterStatus === "pending") {
        return app.status === "pending_teacher"
      }
      if (filterStatus === "approved") {
        return app.status === "approved_teacher" || app.status === "approved" || app.status === "pending_tpo"
      }
      if (filterStatus === "rejected") {
        return app.status === "rejected_teacher" || app.status === "rejected"
      }
      return false
    })
  }
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Internship Application Management</CardTitle>
        <CardDescription>Review and manage internship applications from your students.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-6 flex flex-wrap gap-2">
          <Button variant={activeTab === "pending" ? "default" : "outline"} onClick={() => setActiveTab("pending")}>
            Pending Your Review ({getFilteredApplications("pending").length})
          </Button>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid gap-4 py-4">
              ...
              {selectedApplication.status === "rejected_teacher" && selectedApplication.rejectionReason && (
                <>
                  <Separator />
                  <div>
